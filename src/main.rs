...
fn run_sfml_gui() {
    ...
    let thread = std::thread::spawn(move || {
        if let Some(m) = board_copy.find_best_move(6, rx) {
            println!("Found best move: {:#?}", m);
        } else {
    ...
    let mut window = sfml::graphics::RenderWindow::new((800, 600), "Chess", Style::DEFAULT, &Default::default());

    window.set_vertical_sync_enabled(true);

    let mut piece_sprites  = HashMap::<(Color, Type), Sprite>::new();

    let w_pawn = sfml::graphics::Texture::from_file("ui/icons/w_pawn.png").unwrap();
    let mut sprite = sfml::graphics::Sprite::with_texture(&w_pawn);
